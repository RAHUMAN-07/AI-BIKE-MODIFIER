...
from features.reconstruction.service import generate_3d_mesh
...
def process_reconstruction(session_id: str):
    ...
    try:
        reconstruction_jobs[session_id] = {"status": "processing", "progress": 10}
        ...
        def update_progress(status_stage, pct):
        ...
        try:
            ...
            if sessions_col is not None:
                ...
                print(f"✅ Session {session_id} updated with 3D model URL")
        except Exception as db_err:
            print(f"⚠️ Failed to update session in DB: {db_err}")
...
async def reconstruct_part(request: dict):
    """
    Takes a part name and selected model and generates composited modification preview.
    """
