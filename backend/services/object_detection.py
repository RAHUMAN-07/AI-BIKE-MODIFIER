...
def extract_bike_colors(image_path: str, bbox=None) -> dict:
    ...
    try:
        ...
        # If bbox is given, crop to bbox first to localize the bike
        if bbox:
        ...
        # Helper to get average color in a box defined by percentages
        def get_avg_color(box_pct):
            ...
            # If RGBA (like after background removal), ignore transparent pixels
            ...
            # Standard resize fallback
            rgb_cropped = cropped.convert('RGB')
            one_pix = rgb_cropped.resize((1, 1), Image.Resampling.LANCZOS)
            r, g, b = one_pix.getpixel((0, 0))
            return f"#{r:02x}{g:02x}{b:02x}"

        # Define percentage boxes for parts (optimized for standard side profile)
        # Format: [xmin, ymin, xmax, ymax]
        body_color = get_avg_color([0.35, 0.15, 0.65, 0.40])      # Upper-middle (tank)
        frame_color = get_avg_color([0.30, 0.45, 0.70, 0.70])     # Center (engine/frame/shrouds)
        seat_color = get_avg_color([0.15, 0.35, 0.45, 0.50])      # Upper-rear (seat)
        wheel_color = get_avg_color([0.08, 0.60, 0.28, 0.90])     # Lower-rear (wheel/rim)
        fork_color = get_avg_color([0.72, 0.30, 0.88, 0.65])      # Front-forks (forks)
        exhaust_color = get_avg_color([0.15, 0.50, 0.50, 0.85])   # Exhaust line

        # Build color mapping for useBikeStore
        colors = {
...
def detect_parts(image_path: str):
    ...
    """
    # Defaults/Fallback list in case detection fails, is unavailable, or doesn't find a motorcycle
    default_parts = [
    ...
    # If YOLO not available, we extract colors from the whole image and return fallback parts
    ...
    try:
        # Load the nano detection model — resolve path relative to this file so it
        # works no matter what directory uvicorn is started from.
        ...
        # Find the box with the highest confidence for "motorcycle" (class 3 in COCO)
        motorcycle_box = None
        max_conf = 0.0
        ...
        print(f"Detected motorcycle with confidence {max_conf:.2f}. Bounding box: {motorcycle_box}. Aspect ratio: {aspect_ratio:.2f}")

        # Extract colors based on this bounding box!
        colors = extract_bike_colors(image_path, bbox=motorcycle_box)

        # Core parts always visible on most motorcycles
        ...
        if aspect_ratio > 1.2:
            ...
            part_ids.extend(["handlebar", "mirror_left", "mirror_right"])
        # Front or Rear view
        elif aspect_ratio < 0.9:
            part_ids.extend(["handlebar", "mirror_left", "mirror_right", "headlight", "taillight"])
        # Three-quarter diagonal or standard view
        else:
            part_ids.extend([
        ...
        # Construct response list matching the frontend's expected BIKE_PARTS keys
        detected_parts = []
        for pid in part_ids:
